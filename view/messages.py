...
MSG_MAX_CHOOSE_AUTH = (
    "Выберите способ входа. Для историй нужен SMS: QR даёт web-сессию, "
    "а MAX создаёт сториз только из Android-приложения."
)
MSG_MAX_SCAN_QR = (
    "Откройте MAX → Настройки → Устройства → Подключить устройство и "
    "отсканируйте QR-код. Этот вход подходит для чатов, но не для историй: "
    "MAX не даёт выкладывать сториз с компьютера/web. Для сториз войдите по SMS."
)
MSG_MAX_ASK_PHONE = "Пришлите номер MAX в формате +79001234567."
MSG_MAX_CODE_SENT = (
    "MAX принял запрос кода (6 цифр). Часто это пуш в приложении MAX, "
    "не SMS. Если ничего нет — подождите минуту и нажмите «Ещё раз SMS» "
    "или «Позвонить»."
)
MSG_MAX_CODE_RESENT = "MAX отправил код ещё раз. Проверьте SMS и пуш в MAX."
MSG_MAX_CODE_CALLED = (
    "MAX должен позвонить на этот номер и продиктовать код. "
    "Пришлите цифры сюда."
)
MSG_MAX_ASK_CODE = "Пришлите цифровой код из SMS, пуша MAX или звонка."
...
MSG_MAX_STATUS_WEB = (
    ...
    "сториз в MAX доступны только из Android. "
...
MSG_UNKNOWN_COMMAND = (
    ...
    "/wa_login, /wa_logout, /vk_login, /vk_status, /vk_logout, "
    "/ig_login, /ig_logout."
)
